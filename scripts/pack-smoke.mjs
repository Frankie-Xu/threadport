...
try {
  ...
  execFileSync(process.execPath, ['--input-type=module', '-e', 'const {controlEventSchema}=await import("threadport/control-plane"); const {createContextManifest}=await import("threadport"); const {openStore}=await import("threadport/storage"); if(!controlEventSchema)throw new Error("Missing control-plane export"); const s=await openStore({dataDir:"./data"}); const m=createContextManifest({handoffId:"package-manifest",taskId:"package-task",taskRevision:1}); s.controlPlane().saveManifest(m); if(s.controlPlane().getManifest(m.handoffId)?.digest!==m.digest)throw new Error("Control-plane package persistence failed"); s.close();'], { cwd: installRoot, timeout: 15_000 });
