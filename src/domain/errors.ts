...
export class DomainError extends Error {
  ...
  constructor(readonly code: 'INVALID_INPUT' | 'STORAGE_BUSY' | 'MIGRATION_FAILED' | 'IO_FAILED' | 'REVISION_CONFLICT' | 'INDEX_STALE' | 'INDEX_LIMIT' | 'NOT_FOUND' | 'PROJECT_MISMATCH' | 'REDACTION_REQUIRED' | 'SEARCH_STALE' | 'TARGET_UNSUPPORTED' | 'TARGET_EXITED' | 'CONTEXT_BUDGET_EXCEEDED' | 'WORKSPACE_BUSY' | 'LAUNCH_STATE_UNKNOWN' | 'ASSERTION_CONFLICT' | 'NEXT_ACTION_REVIEW_REQUIRED' | 'RECEIPT_VERIFICATION_REQUIRED' | 'RECEIPT_DIGEST_MISMATCH' | 'RECEIPT_TARGET_MISMATCH' | 'STOP_UNAVAILABLE' | 'CONTROL_COVERAGE_GAP' | 'RECEIPT_NONCE_CONFLICT' | 'TAKEOVER_CONFLICT', message: string) {
    super(message);
