...
export class CodexRunner implements TargetRunner {
 ...
 private async inspect(){const result=await inspect(this.agent,/^codex-cli (\d+\.\d+\.\d+(?:[-+][A-Za-z0-9.-]+)?)$/,['0.154.0-alpha.6.2','0.155.0-alpha.9'],this.options,['resume','--help']);const context=result.help.includes('Usage: codex [OPTIONS] [PROMPT]')&&result.help.includes('--cd <DIR>');const resume=result.extraHelp.includes('Usage: codex resume [OPTIONS] [SESSION_ID] [PROMPT]')&&result.extraHelp.includes('--cd <DIR>');return {result,capability:capability(this.agent,result,context,context&&resume)};}
