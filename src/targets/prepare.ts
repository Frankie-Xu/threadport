...
export async function validateRunnerInput(input:RunnerInput,target:TargetCapability):Promise<void>{
 ...
 if(!target.installed||!(h.mode==='native-resume'?target.nativeResume:target.newSessionWithContext))throw new DomainError('TARGET_UNSUPPORTED','Target capability is unverified; export context instead.');
