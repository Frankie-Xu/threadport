...
export function createJsonlSource(options:SourceOptions & {agent:'claude'|'codex';parserVersion:string;translate?:(row:Record<string,unknown>,metadata:ReadCursor['metadata'])=>Record<string,unknown>}):SourceAdapter{
 const parserVersion=options.parserVersion;
 ...
 return {
  agent:options.agent,parserVersion:parserVersion,
    ...
    while(queue.length){
     ...
     try{
      ...
      for await(const entry of handle){
       ...
       emitted.add(path);yield {sourceId:options.sourceId,path,agent:options.agent};
      }
