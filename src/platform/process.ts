...
export interface ProcessResult {status:'exited'|'failed'|'cancelled'|'interrupted';exitCode:number;errorCode:string|null}
/** Inherit the user's terminal. No shell, command parsing, detached child or background retry. */
export async function runProcess(spec:LaunchSpec,signal?:AbortSignal):Promise<ProcessResult>{
 if(signal?.aborted)return {status:'cancelled',exitCode:130,errorCode:'SIGINT'};
 return new Promise(resolve=>{
  let requested:NodeJS.Signals|undefined;let settled=false;
  ...
  child.once('error',()=>finish({status:'failed',exitCode:5,errorCode:'SPAWN_FAILED'}));
