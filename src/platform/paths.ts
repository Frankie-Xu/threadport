...
export async function privateDirectory(path: string): Promise<void> {
  ...
  if (process.platform !== 'win32') await chmod(path, 0o700);
    ...
    try {
      const script = `
        ...
        try {
          ...
          foreach($r in $acl.Access){
            ...
            $bits=[int]$r.FileSystemRights -band 278
            if($r.AccessControlType -eq 'Allow' -and $s -in @('S-1-1-0','S-1-5-11','S-1-5-32-545') -and $bits -ne 0){exit 3}
            if($s -in $groups){
              if($r.AccessControlType -eq 'Deny' -and $bits -ne 0){exit 4}
              if($r.AccessControlType -eq 'Allow'){$write=$write -bor $bits}
            }
          }
      ...
      const exception = String((error as { stdout?: string }).stdout ?? '').trim();
      const detail = /^[A-Za-z]+Exception$/.test(exception) ? ` ${exception}` : '';
      throw new DomainError('IO_FAILED', `Windows data directory ACL check failed (${typeof reason === 'number' ? reason : 'unavailable'}${detail}). Use a private writable directory.`);
    }
  }
}
