...
export async function confirmHandoff(store:SqliteStore,input:ConfirmInput):Promise<{command:string}>{
 ...
 if(current.snapshot.algorithm!==before.algorithm||current.snapshot.policy!==before.policy||!current.git||current.snapshot.digest!==before.digest||current.snapshot.bindingDigest!==before.bindingDigest||current.snapshot.head!==before.head||current.git.branch!==h.capsule.git.branch)throw new DomainError('REVISION_CONFLICT','Workspace changed after review; prepare again.');
