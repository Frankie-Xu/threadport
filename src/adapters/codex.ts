...
export function createCodexAdapter(): SessionAdapter {
  return { agent: 'codex', async extract(input) {
    ...
    for (const item of items) if (['function_call_output', 'custom_tool_call_output'].includes(String(item.type)) && typeof item.call_id === 'string') {
      if (outputs.has(item.call_id)) throw new Error('Duplicate tool result ID in session.');
      outputs.set(item.call_id, toolResult(item.output, item.is_error === true));
    }
    ...
    for (const item of items) {
      ...
      if (item.type === 'message' && (item.role === 'user' || item.role === 'assistant')) {
        ...
        if (text) events.push({ type: item.role, text });
        continue;
      }
      ...
      if (name === 'apply_patch' || name === 'apply_patch_file') {
        ...
        for (const match of patch.matchAll(/^\*\*\* (Add|Update|Delete) File: (.+)\r?$/gm)) {
          ...
          events.push({ type: 'file', path: match[2].trim(), action: actions[match[1]], outcome: result?.outcome ?? 'unknown', output: result?.text });
        }
      } else if (['shell', 'exec_command', 'local_shell', 'bash'].includes(name)) {
        const command = asString(args.command) ?? asString(args.cmd);
        if (command) events.push({ type: 'command', command, exitCode: result?.exitCode, output: result?.text });
      }
    }
