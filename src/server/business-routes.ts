...
export function registerBusinessRoutes(app:FastifyInstance,store:SqliteStore,indexer:IndexService){
 ...
 app.get('/api/v1/tasks',async request=>{const q=z.object({...pagination,q:z.string().max(1024).default(''),projectId:id.optional(),lifecycle:z.enum(['active','paused','completed']).optional(),archived:z.enum(['true','false']).default('false').transform(value=>value==='true')}).strict().parse(request.query);const page=api.taskPage(q);return{data:page.data.map(taskDto),nextCursor:page.nextCursor};});
 ...
 app.get('/api/v1/sessions/:id/events',async request=>{
  ...
  const q=z.object(pagination).strict().parse(request.query);const generation=api.generation();let offset=0;
