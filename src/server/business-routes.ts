...
export function registerBusinessRoutes(app:FastifyInstance,store:SqliteStore,indexer:IndexService){
 ...
 app.delete('/api/v1/index-jobs/:id',async request=>{pruneJobs();const jobId=p(request),job=jobs.get(jobId);if(!job)throw new DomainError('NOT_FOUND','Job does not exist.');if(request.body!==undefined)empty.parse(request.body);await Promise.all(job.sourceIds.filter(source=>!job.results.has(source)).map(async source=>{await indexer.cancelAndWait(source);const progress=indexer.progress(source);if(progress)job.results.set(source,progress);}));if(job.sourceIds.every(source=>isTerminal(job.results.get(source)?.state??'queued'))){job.finishedAt??=Date.now();jobs.delete(jobId);}return{data:{jobId,cancelled:true}};});
