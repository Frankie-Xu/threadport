...
interface Row {id:string;sessionId:string|null;taskId:string|null;title:string;objective:string;eventText:string;projectId:string|null;projectName:string|null;workspaceId:string|null;workspacePath:string|null;agent:'claude'|'codex'|null;activity:string|null}
...
const documents=`WITH documents AS (
 ...
 coalesce(sp.search_text,'') AS eventText,tp_search_time(s.last_event_at) AS activity
 FROM sessions s LEFT JOIN task_sessions ts ON ts.session_id=s.id LEFT JOIN tasks t ON t.id=ts.task_id
 LEFT JOIN search_session_projection sp ON sp.session_id=s.id LEFT JOIN projects p ON p.id=coalesce(s.project_id,t.project_id) LEFT JOIN workspaces w ON w.id=s.workspace_id LEFT JOIN sources src ON src.id=s.source_id
 UNION ALL
...
export function searchHistory(db:Database.Database,input:SearchInput):SearchPage{
 ...
 return db.transaction(()=>{
  ...
  query.terms.forEach((term,index)=>{values[`term${index}`]=term;clauses.push(`(instr(lower(d.title),@term${index})>0 OR instr(lower(d.objective),@term${index})>0 OR instr(lower(d.eventText),@term${index})>0)`);});
