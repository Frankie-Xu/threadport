...
export async function openDatabase(options: DatabaseOptions = {}): Promise<Database.Database> {
  const dataDir = applicationDataDir(options); let db: Database.Database | undefined;
  try {
    await privateDirectory(dataDir); const path = join(dataDir, 'threadport.sqlite');
    ...
    await migrate(db, dataDir); return db;
