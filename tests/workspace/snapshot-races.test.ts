...
it('verification prioritizes read failure and persistent races over observed drift',async()=>{
 const binding=await setup();const snapshot=await captureWorkspace(binding);
 await writeFile(join(binding.canonicalRoot,'a'),'changed');
 for(const [mode,code] of [['denied','READ_FAILED'],['always','RACED']] as const){
  fault.mode=mode;fault.reads=0;
  const report=await verifyWorkspace(snapshot,binding);
  expect(report).toMatchObject({status:'unverifiable',reasons:[{code}]});
  expect(JSON.stringify(report)).not.toContain('synthetic private error');
  expect(JSON.stringify(report)).not.toContain(binding.canonicalRoot);
 }
},30000);
