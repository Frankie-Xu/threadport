...
it.each([1,10])('normalizes Codex metadata, visible messages and cross-page command results with stable IDs (budget %i)',async(maxEvents)=>{
 ...
 do{page=await source.read({candidate,cursor,maxEvents,signal:new AbortController().signal});cursor=page.cursor;events.push(...page.events);}while(page.hasMore);
...
it.each([1,10])('keeps context changes and malformed/unknown records explicit across pages (budget %i)',async(maxEvents)=>{
 ...
 do{page=await adapter.read({candidate:{agent:'codex',sourceId:'codex',path},cursor,maxEvents,signal:new AbortController().signal});cursor=page.cursor;events.push(...page.events);}while(page.hasMore);
...
it('reads the committed native Codex structural fixture',async()=>{
 ...
 expect(count).toBe(3);expect(page.session.status).toBe('ready');expect(page.events.at(-1)?.commandRun?.exitCode).toBe(0);
});
