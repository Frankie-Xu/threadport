...
const FIXTURE_SESSION_ID = "sess-codex-fixture-basic-001";

async function isolatedProjectRoot(): Promise<string> {
  const root = await mkdtemp(join(tmpdir(), "threadport-codex-"));
...
describe("Codex session adapter", () => {
  it("extracts a valid Capsule v1 from a synthetic Codex session", async () => {
