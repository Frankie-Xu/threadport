...
import { createGeminiAdapter, validateCapsule } from "../../src/index.js";
...
const FIXTURE_SESSION_ID = "sess-gemini-fixture-basic-001";

async function isolatedProjectRoot(): Promise<string> {
  const root = await mkdtemp(join(tmpdir(), "threadport-gemini-"));
...
describe("Gemini session adapter", () => {
  it("extracts a valid Capsule v1 from a synthetic Gemini session", async () => {
