...
it('rolls back injected migration failure and restores consistent WAL-backed user data without overwrite', async () => {
 ...
 await expect(migrate(db, dataDir, [{ version: 5, sql: "DELETE FROM manual; CREATE TABLE partial(x); SELECT * FROM nonexistent;" }])).rejects.toMatchObject({ code: 'MIGRATION_FAILED' });
 ...
 const backup = join(dataDir, 'backups', backups.find(name => name.startsWith('v4-'))!, 'backup.sqlite');
...
it('does not start a migration when backup cannot be written', async () => {
 ...
 await expect(migrate(db, dataDir, [{ version: 5, sql: 'CREATE TABLE lost(x)' }])).rejects.toMatchObject({ code: 'MIGRATION_FAILED' });
 expect(db.pragma('user_version', { simple: true })).toBe(4); db.close();
});

it('applies sequential schema changes once and retains a pre-upgrade backup', async () => {
 const dataDir = await dir(); const db = await openDatabase({ dataDir });
 try {
  await migrate(db, dataDir, [{ version: 5, sql: 'CREATE TABLE upgraded(value TEXT)' }]);
...
it('upgrades the shipped v1 schema without losing manual data and leaves old cursors rebuildable',async()=>{
 ...
 const upgraded=await openDatabase({dataDir});try{expect(upgraded.pragma('user_version',{simple:true})).toBe(4);expect(upgraded.prepare('SELECT body_json FROM tasks').pluck().get()).toBe('{"manual":true}');expect(upgraded.prepare('SELECT task_id FROM task_sessions').pluck().get()).toBe('t');expect(upgraded.prepare('SELECT cursor_json FROM source_cursors').pluck().get()).toBeNull();}finally{upgraded.close();}
});
...
it('upgrades v3 search state transactionally and ignores unchanged projection updates',async()=>{
 ...
 const upgraded=await openDatabase({dataDir});try{
  expect(upgraded.pragma('user_version',{simple:true})).toBe(4);const generation=()=>upgraded.prepare('SELECT generation FROM search_state').pluck().get();expect(generation()).toBe(0);
