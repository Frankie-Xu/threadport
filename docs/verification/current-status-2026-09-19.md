...
- `npm run check`：74 个测试文件、462 个测试通过。
- 浏览器回归：7 个原有流程与 3 个恢复流程通过（共 10 个）。
- 隔离安装包：219 个文件，UI、CLI、SQLite、刷新持久化和 3 次 Cursor 往返通过。
- [完整一键验证报告](./local-validation-final-3-2026-09-19.json)：源码、浏览器和隔离包步骤通过；Linux 容器因宿主无法访问工作区挂载而明确跳过，未伪造成通过。
