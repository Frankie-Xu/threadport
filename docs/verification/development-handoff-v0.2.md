...
2026-09-16 后续本地修复与准确提交、Node24 检查及受限支持见 [整改状态与证据](remediation-progress-2026-09-16.md)。下表保留历史合并证据，不能认证后续代码。数据库现为 schema 11（检索缓存预处理），升级路径见 [存储迁移](../v0.2/07-storage-migration.md) 与 [整合记录](unified-main-refactor-2026-09-20.md)；下方 schema-4 代码往返示例仅为历史记录，不适用于当前数据库。
...
目标兼容性提交 `382b3dc` 已验证 Codex 0.155.0-alpha.9 的版本与 resume 参数形状，未知版本仍安全返回 `unverified`。完整检查为 70 个测试文件/442 个测试，隔离包 217 个文件通过；最新包 SHA-256 为 `a974c274722d8965d8f7fcbf33380fb8833aca745b3dc7fa71898e67c7b78fa5`。这不是登录、权限或真实接续认证。
