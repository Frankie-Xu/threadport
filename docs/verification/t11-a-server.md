...
首轮远端 macOS 在大请求用例再次出现 ECONNRESET，独立连接未完全解决。测试进一步分别验证：chunked 实际传输超过 1 MiB，以及只发送超限 Content-Length 头触发立即 413（不继续发送服务器已拒绝的 payload）。两条均要求实际 HTTP 413，不把 reset 算成功、不改生产限制。修复后目标及默认完整 check 再次通过；后续 CI 结果见 PR。
