...
`threadport targets` 保持旧的纯 PATH 发现输出；`threadport targets --capabilities` 和认证 API `GET /api/v1/targets` 返回 Claude/Codex 新能力 DTO，不含 executable 路径。当前没有实际 continue 执行入口；T14 接入后仍必须完整预览、有效审批、终端再次确认和工作区复验。
