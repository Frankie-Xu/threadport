...
| T17-B 清理和寿命 | feat(T17-B): manage index and retained handoff data | storage/maintenance、index暂停/恢复、server/data-routes、settings、cli-ui关闭信号；生命周期回归 | 清索引保留修订/关联、重建稳定ID、清理取消零副作用、源目录摘要不变、活动扫描/接续拒绝、不重放已消费包、7/30天寿命、删除全部须明确短语并停止服务 | 独立 squash；删除操作本身不具备代码回滚恢复能力，UI先明确影响；恢复需显式选择迁移备份 |
