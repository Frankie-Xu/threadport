...
Follow-up review found that Capsule v1 has no cwd field: internal identity alone left same-command exported rows indistinguishable. Cursor command and test summaries now label the requested directory as execution-unverified, using the existing portable/local path policy before final redaction. Raw commands, numeric-result rules and Capsule v1 are unchanged. The observed sparse JSONL `working_directory` alias is read after explicit `workdir`/`cwd`, before outer-record cwd; explicit null stays unknown. This also prevents an explicit numeric success from resolving a failure requested in a different directory. The alias is Cursor-only; no new Claude format assumption is introduced.
