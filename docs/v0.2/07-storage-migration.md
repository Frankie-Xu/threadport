...
T05 建立 schema v1；T07 通过迁移 002 升至 v2，新增完整 cursor_json 与扫描占用表，见 [增量索引](08-indexing.md)。T08 通过迁移 003 升至 v3，保存完成活动基线和修订关联快照。表与索引见 [契约](03-contracts.md)。sessions 允许 source_id 为空以保留墓碑；saveSession 只保留身份，不是 T06/T07 索引实现。后续来源写入应使用 upsert，不可 REPLACE 删除再插入。数据库启用 WAL、外键和 5000ms busy timeout；锁争用返回可重试 STORAGE_BUSY。查询分页上限 1000。
