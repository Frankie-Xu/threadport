...
诊断和本地目录展示分别使用 GET diagnostics/settings，前者只构造白名单字段，后者只供认证本地 UI。Task/接续导出先预览完整文本和摘要，再在用户选择的已有目录写入固定文件名；摘要变化、文件已存在或磁盘错误均拒绝，临时文件在失败时清理。旧 handoff/export 仍返回浏览器下载字节。

清索引明确暂停所有来源，保留人工修订、会话身份和关联，重新启用后可重建。7 天清包文本/授权，30 天清结束的启动摘要；活动包与依赖快照保留。全部数据删除要求精确短语、拒绝其他登记连接或活动扫描/接续，只删除已核验应用文件并关闭服务；设置页在首次 onboarding 之前也可用。具体跨文件失败边界、旧版本连接前提和恢复方式见 [T17 记录](../verification/t17-data-lifecycle.md)。
