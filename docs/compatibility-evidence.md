...
The sections below retain historical 3.20.10 and early 3.20.17 checkpoints. The [same-version verification](verification/cursor-release-candidate.md) supersedes their uncompleted-gate and uncommitted-delivery statements: actual external-file edit rejection and same-cwd concurrency/reverse completion were exercised in a fresh 3.20.17 session. Three sequential and two parallel probes reproduced all four child-directory requests executing at the root; effective cross-cwd execution remains uncertified. PR #41 is merged as `d35c70c`, and its main CI passed. [Current-main validation hardening](verification/cursor-validation-hardening.md) separately records installed-source replays and SQLite coverage. Settings remained Auto-Review (with Sandbox), with external-file protection on. No full certification or release is claimed.
