...
The sections below retain historical 3.20.10 and early 3.20.17 checkpoints. The [latest same-version verification](verification/cursor-release-candidate.md) supersedes their uncompleted-gate and uncommitted-delivery statements: actual external-file edit rejection and same-cwd concurrency/reverse completion have now been exercised in a fresh 3.20.17 session. A further three-call sequential and two-call parallel comparison reproduced all four child-directory requests executing at the root; effective cross-cwd execution remains uncertified. The candidate integrates main through `6512975` (PR #39), with 311 tests and fresh three-source installed-package roundtrips passing. Settings remain Auto-Review (with Sandbox), with external-file protection on. PR creation/CI inspection is authorized; no full certification, main merge or release is claimed.
