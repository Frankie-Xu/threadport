...
Tests use synthetic fixtures and temporary directories only. Never commit real agent sessions, API keys, or `.env` files. Vitest collects only `tests/**/*.test.ts`, so building `dist/` does not duplicate tests. `npm run test:coverage` instruments `src/**/*.ts` with the V8 provider and enforces a repository baseline of 70% lines/statements/functions and 60% branches. New pure domain modules should target at least 85% branch coverage; named regression scenarios remain mandatory even when aggregate coverage is higher.
